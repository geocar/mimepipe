...
/* $Id: mm_base64.c,v 1.1.1.1 2004/05/03 22:05:56 jfi Exp $ */
...
/*
 * mm_base64_decode_loc()
 *
 * Decodes the mm_loc indicated by 'data' from the BASE64 encoding to the data
 * format it was encoded from. Returns a pointer to a string on success or
 * NULL on error. The string returned needs to be freed by the caller at some
 * later point.
 *
 * if "out" is NOT NULL, then the semantics change greatly to simplify the
 * fact that this is a driver-function: the content is written directly to out,
 * and (char*)NULL is returned on failure- something OTHER THAN (char*)NULL will
 * be returned on success of decoding- but, ferror(out) should still be checked
 * at completion.
 *
 */
char *
mm_base64_decode_loc(struct mm_loc *ll, FILE *out)
{
	char *buf, *data;
	int a, b, c, d;
	size_t n;
...
#define RC(NN) do { \
	NN = fgetc(in);n++; \
	if (feof(in) || NN == EOF || n >= ll->len) NN = '='; \
} while (NN != '=' && CHAR64(NN) == XX); NN=CHAR64(NN)
		RC(a);
		RC(b);
		RC(c);
		RC(d);
